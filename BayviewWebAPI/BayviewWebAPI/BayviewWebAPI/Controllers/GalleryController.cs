...
namespace RealBridgeWebAPI.Controllers
{
    ...
    {
        private readonly IGalleryService _imagesService;

        public GalleryController(IGalleryService imagesService)
        {
        ...
        public async Task<HttpResponseMessage> PostAsync()
        {
            ...
            if (httpRequest.Files.Count > 0)
            {
                try
                {
                    ...
                    Image image = new Image { Title = formData.Get("title"), Description = formData.Get("description"), ImageData = imageBuffer };

                    List<Image> images = await _imagesService.AddImage(image).ConfigureAwait(false);
